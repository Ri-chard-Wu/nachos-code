...
// #define StartupPCState  (_ECX/4-1) // 2
    ...
    machineState[PCState] = (void*)ThreadRoot; // 7 
    machineState[StartupPCState] = (void*)ThreadBegin; // 2
    machineState[InitialPCState] = (void*)func; // 5
    machineState[InitialArgState] = (void*)arg; // 3
    machineState[WhenDonePCState] = (void*)ThreadFinish; // 6
#endif
}
...
Thread::RestoreUserState()
{
    for (int i = 0; i < NumTotalRegs; i++) // (machine.h) NumTotalRegs == 40
