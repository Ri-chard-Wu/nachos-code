...
void Kernel::ExecAll()
{
	for (int i=1;i<=execfileNum;i++) {
		int a = Exec(execfile[i]);    // create and put thread into ready list, not yet runned.
	}
	currentThread->Finish(); // first thread is `main()`. stop it now and switch to `halt()`.
    //Kernel::Exec();	
}
...
{
	t[threadNum] = new Thread(name, threadNum); // `name`: name of the executable, e.g. `halt`.
